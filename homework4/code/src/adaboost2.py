...
class DecisionStump():
    ...
    def fit(self,X,D,y):
        ...
        for j in range(d):
            ...
            for i in range(m):
                Fpos -= tempY[i] * tempD[i]
                Fneg += tempY[i] * tempD[i]
                if (Fpos < Fs or Fneg < Fs) and (tempArr[i] != tempArr[i+1]):
...
class MyAbaboost():
    ...
    def fit(self,X_train,y_train,X_test,y_test,verbose = False):
        ...
        for i in range(self.n_estimators):
            ...
            epsilon = np.sum(D[y_train != y_pred])
            w = 0.5 * np.log(1 / (epsilon + np.finfo(np.float32).eps) - 1)

            tempArr = D * np.exp(-w * y_train * y_pred)
            for j in range(m):
